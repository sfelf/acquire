...
Current coverage is informational while golden replay tests are expanded before the major refactor. The latest local baseline is 76% total server coverage, with `coverage.xml` generated by the coverage command and CI coverage step.
