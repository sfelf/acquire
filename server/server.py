...
class Client:
    ...
    def __init__(
        ...
        def output_connect_messages():
            ...
            self._server.transport_write(
                b"connect " + ujson.dumps([socket_id, self.client_id]).encode() + b"\n"
            )
        ...
        if self.username in self._server.username_to_client:
            if replace_existing_user:
                ...
                messages_client.append(
                    [
                        enums.CommandsToClient.FatalError.value,
                        enums.Errors.UsernameAlreadyInUse.value,
                    ]
                )
        ...
        for client in self._server.client_id_to_client.values():
            if client is not self:
                messages_client.append(
                    [
                        enums.CommandsToClient.SetClientIdToData.value,
                        client.client_id,
                        client.username,
                        client.ip_address,
                    ]
    ...
    def _on_message_create_game(self, mode: int, max_players: int) -> None:
        ...
        if (
            ...
            and 0 <= mode < enums.GameModes.Max.value
            and isinstance(max_players, int)
            and 1 <= max_players <= 6
        ):
        ...
        while pending:
            ...
            for coords in pending:
                ...
                for coords2 in possibilities:
                    if (
                        coords2 not in found
                        and self.x_to_y_to_board_type[coords2[0]][coords2[1]]
                        not in excluded_board_types
                    ):
            ...
            if player_id != client.player_id:
                ...
                messages_client.append(
                    [
                        ...
                        x,
                        y,
                        enums.GameBoardTypes.NothingYet.value,
                    ]
                )
        ...
        self.game.add_pending_messages(
            [
                [
                    ...
                    player_id,
                    score_sheet_index,
                    self.player_data[player_id][score_sheet_index],
                ]
            ],
    ...
    def set_chain_size(self, game_board_type_id: int, chain_size: int) -> None:
        ...
        self.game.add_pending_messages(
            [
                [
                    enums.CommandsToClient.SetScoreSheetCell.value,
                    enums.ScoreSheetRows.ChainSize.value,
                    game_board_type_id,
                    chain_size,
                ]
            ],
    ...
    def get_bonuses(self, game_board_type_id):
        ...
        if len(player_id_sets) == 1 and len(player_id_sets[0]) == 1:
            ...
            bonus_data.append(
                [
                    player_id_sets[0],
                    math.ceil((bonus_first + bonus_second) / len(player_id_sets[0])),
                ]
            )
        else:
            ...
            if len(player_id_sets[1]) == 1:
                ...
                bonus_data.append(
                    [
                        player_id_sets[1],
                        math.ceil(bonus_second / len(player_id_sets[1])),
                    ]
                )
...
class TileRacks:
    ...
    def determine_tile_game_board_types(self, player_ids=None):
        ...
        for player_id in player_ids:
                    ...
                    if old_type is None:
                        ...
                        if client_ids:
                            self.game.add_pending_messages(
                                [
                                    [
                                        enums.CommandsToClient.SetTile.value,
                                        tile_index,
                                        x,
                                        y,
                                        new_type,
                        ...
                        if client_ids:
                            self.game.add_pending_messages(
                                [
                                    [
                                        enums.CommandsToClient.SetTileGameBoardType.value,
                                        tile_index,
                                        new_type,
                                    ]
                                ],
            ...
            if drew_last_tile:
                self.game.add_history_message(
                    enums.GameHistoryMessages.DrewLastTile.value, player_id
                )
        ...
        while replaced_a_dead_tile:
            ...
            for tile_index, tile_data in enumerate(rack):
                if tile_data and tile_data[1] == enums.GameBoardTypes.CantPlayEver.value:
                    ...
                    if client:
                        self.game.add_pending_messages(
                            [[enums.CommandsToClient.RemoveTile.value, tile_index]],
                            {client.client_id},
                        )
...
class Action:
    ...
    def send_message(self, client_ids: set[int]) -> None:
        ...
        self.game.add_pending_messages(
            [
                [
                    enums.CommandsToClient.SetGameAction.value,
                    self.game_action_id,
                    self.player_id,
                ]
                + self.additional_params
...
class ActionPlayTile(Action):
    ...
    def prepare(self):
        ...
        if has_a_playable_tile:
            ...
            self.game.add_history_message(
                enums.GameHistoryMessages.HasNoPlayableTile.value, self.player_id
            )
            return True
    ...
    def execute(self, tile_index):
        ...
        if game_board_type_id <= enums.GameBoardTypes.Imperial.value:
            ...
            game_board_type_id == enums.GameBoardTypes.WillPutLonelyTileDown.value
            or game_board_type_id == enums.GameBoardTypes.HaveNeighboringTileToo.value
        ):
            ...
            retval = [
                ActionSelectNewChain(
                    self.game,
                    self.player_id,
                    [
                        index
                        for index, size in enumerate(self.game.score_sheet.chain_size)
                        if size == 0
                    ],
                    tile,
                )
        ...
        self.game.add_history_message(
            enums.GameHistoryMessages.PlayedTile.value, self.player_id, tile[0], tile[1]
        )
...
class ActionSelectMergerSurvivor(Action):
    ...
    def _prepare_next_actions(self, controlling_type_id):
        ...
        self.game.score_sheet.set_chain_size(
            controlling_type_id,
            len(self.game.game_board.board_type_to_coordinates[controlling_type_id]),
        )
        self.game.tile_racks.determine_tile_game_board_types()
        ...
        for type_id in sorted(type_ids):
            for player_ids, bonus in self.game.score_sheet.get_bonuses(type_id):
                for player_id in sorted(player_ids):
                    ...
                    self.game.add_history_message(
                        enums.GameHistoryMessages.ReceivedBonus.value,
                        player_id,
                        type_id,
                        bonus,
                    )
        ...
        for type_id_set in self.type_id_sets:
            if type_id_set:
                actions.append(
                    ActionSelectChainToDisposeOfNext(
                        self.game, self.player_id, type_id_set, controlling_type_id
                    )
                )
    ...
    def _prepare_next_actions(self, next_type_id):
        ...
        for player_id in player_ids:
            if self.game.score_sheet.player_data[player_id][next_type_id]:
                actions.append(
                    ActionDisposeOfShares(
                        self.game, player_id, next_type_id, self.controlling_type_id
                    )
                )
...
class ActionDisposeOfShares(Action):
    ...
    def __init__(self, game, player_id, defunct_type_id, controlling_type_id):
        ...
        self.controlling_type_id = controlling_type_id
        self.defunct_type_count = self.game.score_sheet.player_data[self.player_id][
            self.defunct_type_id
        ]
    ...
    def execute(self, trade_amount, sell_amount):
        ...
        if (
            ...
            or trade_amount < 0
            or trade_amount % 2 != 0
            or trade_amount // 2 > self.controlling_type_available
        ):
            return
        ...
        if trade_amount or sell_amount:
            self.game.score_sheet.adjust_player_data(
                self.player_id, self.defunct_type_id, -trade_amount - sell_amount
            )
            if trade_amount:
                self.game.score_sheet.adjust_player_data(
        ...
        self.game.add_history_message(
            enums.GameHistoryMessages.DisposedOfShares.value,
            self.player_id,
            self.defunct_type_id,
            trade_amount,
            sell_amount,
...
class ActionPurchaseShares(Action):
    ...
    def prepare(self):
        ...
        if not can_purchase_shares and not self.can_end_game:
            if self.can_not_afford_any_shares:
                self.game.add_history_message(
                    enums.GameHistoryMessages.CouldNotAffordAnyShares.value,
                    self.player_id,
                )
    ...
    def execute(self, game_board_type_ids, end_game):
        ...
        for game_board_type_id, count in game_board_type_id_to_count.items():
            if (
                self.game.score_sheet.chain_size[game_board_type_id]
                and count <= self.game.score_sheet.available[game_board_type_id]
            ):
        ...
        if self.can_not_afford_any_shares:
            self.game.add_history_message(
                enums.GameHistoryMessages.CouldNotAffordAnyShares.value, self.player_id
            )
        else:
            self.game.add_history_message(
                enums.GameHistoryMessages.PurchasedShares.value,
                self.player_id,
                sorted(list(x) for x in game_board_type_id_to_count.items()),
            )
    ...
    def _complete_action(self):
        ...
        if self.end_game or all_tiles_played or no_tiles_played_for_entire_round:
            ...
            return [
                ActionPlayTile(self.game, next_player_id),
                ActionPurchaseShares(self.game, next_player_id),
            ]
...
class Game:
    ...
    def do_game_action(self, client, game_action_id, data):
        ...
        if (
            ...
            and client.player_id == action.player_id
            and game_action_id == action.game_action_id
        ):
    ...
    def _send_past_history_messages(self, client):
        ...
        if messages:
            self.add_pending_messages(
                [[enums.CommandsToClient.AddGameHistoryMessages.value, messages]],
                {client.client_id},
            )
    ...
    def _send_initialization_messages(self, client):
        ...
        messages = [
            [
                ...
                self.game_board.x_to_y_to_board_type,
            ]
        ]
