"""Copy Acquire tables from MySQL into Postgres without runtime ORM loading."""
...
from acquire.migration.schema import CURRENT_TARGET_TABLES, LEGACY_SOURCE_TABLES
...
def import_engines(
    ...
    *,
    ...
    source_tables = _ordered_tables(LEGACY_SOURCE_TABLES, TABLE_ORDER)
    target_tables = _ordered_tables(CURRENT_TARGET_TABLES, TABLE_ORDER)

    with source_engine.connect() as source_connection, target_engine.begin() as target_connection:
        _validate_source_tables(source_connection, source_tables)
        ...
        for source_table, target_table in zip(
            ...
            rows = _read_rows(source_connection, source_table)
            target_rows = _read_rows(target_connection, target_table)
            _validate_target_table(target_table, rows, target_rows)
            ...
            if not dry_run and target_count != source_count:
                raise ImportValidationError(
                    f"{target_table.name} imported {target_count} rows; expected {source_count}"
                )
            if not dry_run:
                _reset_postgres_sequence(target_connection, target_table)
            results.append(
                TableImportResult(
...
def _ordered_tables(
    ...
    """Return migration-schema tables in the requested copy order.

    Args:
        tables_by_name: Explicit source or target tables keyed by name.
        table_names: Application table names in foreign-key-safe copy order.

    Returns:
        Tuple of SQLAlchemy table objects.
    """
    return tuple(tables_by_name[table_name] for table_name in table_names)
