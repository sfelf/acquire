...
from acquire import auth, enums, orm, realtime
...
def encode_fatal_error(
    error: enums.Errors,
    *,
    encode_messages: Callable[[str], str] = realtime.encode_sockjs_messages,
) -> str:
...
def create_app(
    *,
    ...
    realtime_gateway: realtime.SockJSGateway | None = None,
    sockjs_heartbeat_interval: float = realtime.SOCKJS_HEARTBEAT_INTERVAL_SECONDS,
    expired_game_cleanup_interval: float = (
        realtime.EXPIRED_GAME_CLEANUP_INTERVAL_SECONDS
    ),
) -> FastAPI:
    ...
    """
    gateway = realtime_gateway or realtime.SockJSGateway()
    app = FastAPI(
    ...
    async def serve_websocket_connection(
        ...
        *,
        ...
        except realtime.DuplicateSessionIdError:
    ...
    async def sockjs_raw_websocket(websocket: WebSocket) -> None:
        ...
        await serve_websocket_connection(
            ...
            decode_frame=realtime.decode_raw_websocket_frame,
    ...
    async def sockjs_websocket(
        ...
        authenticated commands into `game_server.Client`.
        ...
        await serve_websocket_connection(
            ...
            decode_frame=realtime.decode_sockjs_frame,
            encode_messages=realtime.encode_sockjs_messages,
            open_frame=realtime.SOCKJS_OPEN_FRAME,
            heartbeat_frame=realtime.SOCKJS_HEARTBEAT_FRAME,
        )
