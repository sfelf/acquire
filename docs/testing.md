...
The `postgres` marker starts a disposable Docker Compose Postgres service when
`ACQUIRE_POSTGRES_TEST_URL` is not set. Set `ACQUIRE_POSTGRES_TEST_URL` only
when you want the tests to use an existing disposable test schema. The `e2e`
marker generates client assets with the opt-in client build helper, then starts
the local Compose stack with the Python gateway when `ACQUIRE_E2E_URL` is not
set. Set `ACQUIRE_E2E_URL` only when you want the tests to use an existing
local stack.
