...
| --- | --- | --- | --- |
| Host and port | Operator or container configuration, untrusted | Resolve hostnames across IPv4 and IPv6, then bind and start every usable Uvicorn listener | Validate the TCP port; use fixed diagnostics for invalid input and listener failures |
