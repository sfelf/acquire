...
- Current branch with `acquire-migrate-mysql-to-postgres`,
  `acquire-validate-migration-reports`, their focused unit tests, and the
  Postgres/e2e marker suites passing.
...
- The actual import JSON report matches source and target row counts for every
  copied table.
- `acquire-validate-migration-reports` accepts the dry-run/import report pair.
