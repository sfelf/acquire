...
- Client assets live under `client/` and are generated by the opt-in `client-build` Compose profile or the legacy shell scripts.
...
- Do not add Node.js services back to the default local runtime path; keep Node usage limited to explicit legacy compatibility or client asset build profiles until the frontend toolchain is modernized.
...
The legacy runtime and client asset build helpers still use:
